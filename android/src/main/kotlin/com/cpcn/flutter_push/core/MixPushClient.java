...
import com.cpcn.flutter_push.core.utils.ProcessUtils;
import com.cpcn.flutter_push.oppo.OppoPushProvider;
...
public class MixPushClient {
    ...
    public void register(Context context, String defaultPlatform, String passThroughPlatform) {
        ...
        addPlatformProviderByClassName("com.cpcn.flutter_push.mi.MiPushProvider");
//        addPlatformProviderByClassName("com.cpcn.flutter_push.meizu.MeizuPushProvider");
        addPlatformProviderByClassName("com.cpcn.flutter_push.huawei.HuaweiPushProvider");
        addPlatformProviderByClassName("com.cpcn.flutter_push.vivo.VivoPushProvider");
        addPlatformProviderByClassName("com.cpcn.flutter_push.oppo.OppoPushProvider");
        addPlatformProviderByClassName("com.cpcn.flutter_push.honor.HonorPushProvider");
    ...
    public void getRegisterId(Context context, final GetRegisterIdCallback callback, boolean isPassThrough) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                getHandler().getLogger().log(TAG,"———————————————————————————————————————————getRegisterId—start—————————————————————————————————————————");
                String regId = pushProvider.getRegisterId(appContext);

                getHandler().getLogger().log("getRegisterId","————————pushProvider.getRegisterId"+regId   +"MixPushClient.regId "+MixPushClient.regId);
                if (regId!= null && !regId.isEmpty()) {
                    callback.callback(new MixPushPlatform(pushProvider.getPlatformName(), regId));
                    return;
                }else{
                    if (MixPushClient.regId!= null && !MixPushClient.regId.isEmpty()) {
                        callback.callback(new MixPushPlatform(pushProvider.getPlatformName(), MixPushClient.regId));
                        return;
                    }
                }
                handler.postDelayed(this, 2000);
    ...
    public void unRegister(Context context) {
        if (notificationPushProvider != null) {
            notificationPushProvider.unRegister(context);
        }
    }
    public void turnOnPush(Context context) {
        if (notificationPushProvider != null) {
            notificationPushProvider.turnOnPush(context);
        }
    }
    public void turnOffPush(Context context) {
        if (notificationPushProvider != null) {
            notificationPushProvider.turnOffPush(context);
        }
    }

    public boolean isRegisteredForRemoteNotifications() {
        if (notificationPushProvider != null) {
            return notificationPushProvider.isTurnOnPush();
        } else {
            return false;
        }
    }
